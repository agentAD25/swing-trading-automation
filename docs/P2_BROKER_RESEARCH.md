...
| Current official evidence | TS1, “Place Order,” “Replace Order,” and “Cancel Order,” accessed 2026-09-26: replace modifies an active order and returns an acknowledgement; cancel “Sends a cancellation request to the relevant exchange”; endpoint responses include 503/504. No recovery or safe-retry algorithm is documented. |
| Known / unknown | **Known:** command responses can be acknowledgements rather than final state. **Unknown:** commit point, lookup key, retry safety, race outcomes, and authoritative REST/stream recovery sequence. |
...
| Downstream blockers | `G1`; environment scoping in `P2-C`. |
...
| Known / unknown | **Known:** some group processing is explicitly non-atomic and partial response shapes exist. **Unknown:** commit/rollback boundary by group type, OSO parent/child failure behavior, ordering, and recovery from mixed/ambiguous outcomes. |
...
| Downstream blockers | `P2-D` retry/backoff design; `P2-B`/`P2-C` topology. |
